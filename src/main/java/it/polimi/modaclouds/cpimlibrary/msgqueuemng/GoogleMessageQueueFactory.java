...
class GoogleMessageQueueFactory extends CloudMessageQueueFactory {

	private HashMap<String, QueueInfo> info = null;
	
	public GoogleMessageQueueFactory(CloudMetadata metadata) {
	...
	public CloudMessageQueue getQueue(String queueName) {
		...
		if (queueInfo.getMode().equals(ModeQueue.PULL))
			return new GoogleMessageQueue(queueName);
		try {
